...
def global_coordinate_frame_kinect(kp_arr, kp_ind):
    # print(kp_arr)
    # quit()
    left_hip = kp_arr[kp_ind["Hip-Left"]]
    right_hip = kp_arr[kp_ind["Hip-Left"]]
...
def keypoints_to_unit_vectors(kp_arr, kp_ind, pc):
    unit_vectors = []
    for p in pc:
        p1_ind, p2_ind = p
...
def body_part_feature_lengths(kp_arr, pc):
    """
...
def inter_frame_angles(kp_arr1, kp_arr2, kp_ind, pc):
    if kp_arr2 is None:
        ret_zeros = []
        for i in range(len(kp_arr1)):
            ret_zeros.append(0)
        return ret_zeros
    angles1 = keypoints_to_unit_vectors(kp_arr1, kp_ind, pc)
    angles2 = keypoints_to_unit_vectors(kp_arr2, kp_ind, pc)
    d_angles = []
...
def preprocess_kinect_dataset(x, y, kp_ind, pc):
    glob_coords_x = x#global_coordinate_frame(x)
    glob_coords_y = y#y if y is None else global_coordinate_frame(y)
    return [
        keypoints_to_unit_vectors(glob_coords_x, kp_ind, pc),
        body_part_feature_lengths(glob_coords_x, pc),
        join_distance_features(glob_coords_x),
        inter_frame_distances(glob_coords_x, glob_coords_y),
        inter_frame_angles(glob_coords_x, glob_coords_y, kp_ind, pc)
    ]
