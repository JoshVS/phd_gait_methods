...
def get_kp_from_video(filename, kp_dict, annotations=None, min_frames = 2, im_height=256, im_width=256, max_people=10):    
    """
    ...
    frame_results = [[]] * max_people
    
    loop = tqdm(range(frame_counter), desc="Extracting Keypoints")
...
class SuspiciousDataset(GenericGaitDataset):
    ...
    def convert_to_one_hot(self):
        onehot_mat = np.zeros((self.y.shape[0], self.n_classes))
    ...
    def reshape_skeletons(self, skel_data=None, unmash_kp = False):
        ...
        if not os.path.exists(os.path.join(self.directory, "pickled_objects", "reshaped")):
            os.makedirs(os.path.join(self.directory, "pickled_objects", "reshaped"))
        reshaped_data = []
