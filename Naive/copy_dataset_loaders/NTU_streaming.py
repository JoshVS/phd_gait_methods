...
def read_from_cached_file(filename, min_samples=2):
    ...
    for line in lines:
        ...
        curr_data.append([int(i), kp, float(x), float(y), float(z)])
...
def get_kp_from_file(filename, kp_dict, min_frames = 2, im_height=256, im_width=256):
    ...
    frames = open(filename, 'r').read().split('\n')
    # bounding_boxes = yolo_model([os.path.join(filename, f) for f in frames], save=False, verbose=False)
    num_frames = int(frames[0])
    vid_results = [
        [0, 'null', 0,0,0]
    ] * (num_frames + 1) * 25
    ...
    for i in range(1, len(frames)):
        ...
        if frame_num >= (num_frames + 1):
            break
        # print(joint_num, (num_frames + 1), frame_num, curr_line)
        x, y, z = map(float, line_data[:3])
        # print(list(kp_dict.values()))
        # quit()
        k = list(kp_dict.keys())[list(kp_dict.values()).index(joint_num)]
        # vid_results[frame_num][-(125 - (joint_num*5) - 5)] = 1
        # vid_results[frame_num][-(125 - (joint_num*5) - 4)] = k
        # vid_results[frame_num][-(125 - (joint_num*5) - 3)] = x
        # vid_results[frame_num][-(125 - (joint_num*5) - 2)] = y
        # vid_results[frame_num][-(125 - (joint_num*5) - 1)] = z
        
        vid_results[(frame_num) * 25 + joint_num] = [
            0,
            k,
            x,
            y,
...
class NTURGBDDataset(GenericGaitDataset):
    ...
    def load_from_streaming(self, batch_size=32):
        if not os.path.exists("streaming"):
            os.makedirs("streaming")
        for i in range(self.X.shape[0] // batch_size):
            np.save(os.path.join("streaming", f"X_{i}.pkl"), self.X[32 * i: 32 * i + 32])
            np.save(os.path.join("streaming", f"y_{i}.pkl"), self.y[32 * i: 32 * i + 32])
        ...
        for i, person in enumerate(loop):
            ...
            for l in person:
                ...
                if l[1] == self.headpoint:
                    reshaped_skel[-1][0].append([])
                reshaped_skel[-1][0][-1].append(l[2:])
    ...
    def create_file_data(self, kp_dict, max_samples, max_classes):
        ...
        z_data = []
        vid_filenames = []
        sample_names = os.listdir(self.directory)
